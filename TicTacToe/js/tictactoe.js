...
function placeXOrO(squareNumber) { //function to place the x or o in the square
  if (!selectedSquares.some(element => element.includes(squareNumber))) {
    ...
    selectedSquares.push(squareNumber + activePlayer); //adds to the array
    checkWinConditions();
    if (activePlayer === 'X') {
      activePlayer = 'O';
    } else {
    ...
    audio('./media/click.mp3'); //plays the click sound
    if (activePlayer === 'O') { //checks if computers turn
      disableClick();
...
function computersTurn() {  //computer picks random square
  let success = false;
  let pickASquare;
  while (!success) {
...
function checkWinConditions() { //all the win conditions
  if (arrayIncludes('0X', '1X', '2X')) {
    ...
    audio('./media/meow.mp3'); //play audio if tie 
    setTimeout(function () { resetGame(); }, 500);
  }


    function arrayIncludes(squareA, squareB, squareC) { //checks for the win
    ...
    function disableClick() {  //so you cant click if someone wins or ties
    body.style.pointerEvents = 'none';
    setTimeout(function () { body.style.pointerEvents = 'auto'; }, 1000);
    }

    function audio(audioURL) { //play the audio
    let audio = new Audio(audioURL);
    audio.play();
    }

    function drawWinLine(coordX1, coordY1, coordX2, coordY2) { //this draws the lines
    ...
    function animateLineDrawing() { //adjusts the lines 
    ...
    disableClick(); //plays the win audio
    audio('./media/win.mp3');
    animateLineDrawing();
    setTimeout(function () { clear(); resetGame(); }, 1000)
    }
    function resetGame() { //resets the game when it ends
    for (let i = 0; i < 9; i++) {
